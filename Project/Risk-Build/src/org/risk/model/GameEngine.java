...
public class GameEngine {
	...
	private static ArrayList<Player> playerTurnSequence;
	private ArmyFactory armyFactory;
	private Timer timer;
	private static ArrayList<Player> nextPlayerTurn;
	...
	public GameEngine() {
		...
		player = new Player();
		if (playerTurnSequence == null)
			playerTurnSequence = new ArrayList<Player>();
	...
	private void populateMap() {
		...
		myPlayerList = map.getMyPlayerList();
		myStateList = map.getMyStateList();
		myLinkList = map.getMyLinkList();
		nextPlayerTurn = map.getNextPlayerTurn();
		playerTurnSequence = map.getPlayerTurnSequence();
		stage = map.getStage();
	}
	...
	public void analyzeAttack(int countryID, boolean firstAnalysis)
		...
		while (!canAttack && !enemyUnavailable) {
			canAttack = canAttack(countryID, myMap, WeakestStateIndex,
					canAttack, enemyStates, stateStrengths);
	...
	private boolean canAttack(int countryID, Map myMap, int WeakestStateIndex,
			boolean canAttack, ArrayList<State> enemyStates,
			float[] stateStrengths) {
		WeakestStateIndex = getWeakestStateIndex(stateStrengths);
		if (WeakestStateIndex != -1) {
			ArrayList<State> adjStates = new ArrayList<State>();
			adjStates = myMap.getAdjacentStatesOfState(
			...
			if (attackingStrength >= enemyStates
					...
					.getArmyDetail()
					.getArmyStrength(
							enemyStates.get(WeakestStateIndex).getArmyDetail())) {
				canAttack = true;
			}
		}
		return canAttack;
	 ...
	 *         turn
	 */
	public ArrayList<Player> playerTurnSequence(ArrayList<Player> playerList) {
		int randomNumber = 0;
		int countryListLength = playerList.size();
		/**
		...
		playerTurnSequence.removeAll(playerTurnSequence);
		...
		for (int counter = 0; counter < countryListLength;) {
			...
			if (validateArray[randomNumber] == false) {
				...
				playerTurnSequence.add(playerList.get(randomNumber));
				counter++;
			} else {
				randomNumber = rand.nextInt(countryListLength);
			}
		}
		nextPlayerTurn = (ArrayList<Player>) playerTurnSequence.clone();
	 ...
	 *            : Arraylist of links
	 */
	public void startUpPhase(ArrayList<Player> playerList,
		...
		this.map.setMyPlayerList(this.myPlayerList);
		...
		ArrayList<Player> tempPlayerArr = (ArrayList<Player>) this.myPlayerList
				.clone();
		playerTurnSequence(tempPlayerArr);
		this.map.setPlayerTurnSequence(this.playerTurnSequence);
		this.map.setNextPlayerTurn(this.nextPlayerTurn);
	 ...
	 */
	public void startUpPhase(Map map, ArrayList<Player> playerList)
			throws Exception {
		...
		myStateList = player.removeCapitals(map.getMyStateList());
		...
		map.assignStateToCountry(myStateList, myPlayerList, myLinkList);
		this.map.setMyPlayerList(playerList);
		myStateList = map.getMyStateList();

		if (playerList.size() <= myStateList.size()) {
			// Assign a capital to each country
			for (Player newPlayer : myPlayerList) {
				tempStateArr = state.getStatesByCountryId(
						newPlayer.getCountryID(), myStateList);
			...
			assignTechnologyLevelToCountry();
			assignResourcesToStates();
			playerTurnSequence(myPlayerList);
			this.map.setPlayerTurnSequence(this.playerTurnSequence);
			this.map.setNextPlayerTurn(this.nextPlayerTurn);
	...
	public void startUpPhaseSameSimulation(Map map) {
		...
		if (playerTurnSequence.size() == 0 && nextPlayerTurn.size() == 0) {
			assignTechnologyLevelToCountry();
			assignResourcesToStates();
			playerTurnSequence(myPlayerList);
		}
	...
	private void assignTechnologyLevelToCountry() {
		...
		for (Player player : myPlayerList) {
			player.setTechnology(technology);
		}
	}
	...
	private void runSimulationSmoothly() throws Exception {
		...
		if (!status) {
			...
			timer.schedule(new TimerTask() {
				...
				{
					if (!gameStatus(false)) {
						Player nextPlayer = nextPlayerTurn.get(0);
						int countryId = nextPlayer.getCountryID();
						int technologyLevel = nextPlayer.getTechnology()
								.technologyLevelNo();
						...
						case Constants.STRATEGIC_MOVEMENT_STAGE:
							...
							GameEngine.nextPlayerTurn = incrementPlayerTurnAndTechnology(
									countryId, nextPlayerTurn);
							stage = Constants.PRODUCTION_STAGE;
							map.setStage(stage);
							maintainCountryTurn();
						}
					} else {
						JOptionPane.showMessageDialog(game, nextPlayerTurn
								.get(nextPlayerTurn.size() - 1)
								.getCountryName()
								+ " wins the game");
	...
	private void runSimulationByCountryTurn() throws Exception {
		...
		if (!status) {
			...
			JOptionPane.showMessageDialog(game,
					nextPlayerTurn.get(nextPlayerTurn.size() - 1)
	...
	private void runSimulationByAllCountriesTurn() throws Exception {
		boolean status = false;
		for (int i = 0; i < playerTurnSequence.size(); i++) {
			status = gameStatus(true);
			if (!status) {
	...
	private void runSimulationByEachPhase() throws Exception {
		...
		Player nextPlayer = nextPlayerTurn.get(0);
		int countryId = nextPlayer.getCountryID();
		int technologyLevel = nextPlayer.getTechnology().technologyLevelNo();
		if (!status) {
			switch (stage) {
			...
			case Constants.STRATEGIC_INTELLIGENCE_STAGE:
				// Call Strategic Intelligence Gathering Stage
				analyzeAttack(nextPlayer.getCountryID(), true);
	...
	private void maintainCountryTurn() {
		ArrayList<Player> tempArr = new ArrayList<Player>();
		tempArr = (ArrayList<Player>) nextPlayerTurn.clone();
		Player player = new Player();
		int index = 0;
		if (nextPlayerTurn.size() > 0) {
			nextPlayerTurn.removeAll(nextPlayerTurn);
			nextPlayerTurn = new ArrayList<Player>();
			for (int i = 0; i < tempArr.size() - 1; i++) {
				if (player.isCountryHaveCapital(tempArr.get(i + 1)
						.getCountryID(), this.map.getMyStateList())) {
					nextPlayerTurn.add(index, tempArr.get((i + 1)));
					index++;
				}
			}
			if (player.isCountryHaveCapital(tempArr.get(0).getCountryID(),
					this.map.getMyStateList())) {
				nextPlayerTurn.add(nextPlayerTurn.size(), tempArr.get(0));
			}
			this.map.setNextPlayerTurn(nextPlayerTurn);
		}
	}
	 ...
	 */
	private ArrayList<Player> incrementPlayerTurnAndTechnology(int countryId,
			ArrayList<Player> playerArr) {
		try {
			Player player = new Player();
			Technology technology = new Technology();
			int index = player.getPlayerIndexByCountryID(countryId,
					playerArr);
			player = playerArr.get(index);
			int countryTurn = player.getCountryTurn();
			if (countryTurn == 5) {
				player.setTechnology(technology.technologyLevelMedium());
			} else if (countryTurn == 10) {
				player.setTechnology(technology.technologyLevelHigh());
			}
			player.setCountryTurn(countryTurn + 1);
			playerArr.set(index, player);
		} catch (Exception ex) {
	...
	private boolean gameStatus(boolean printResult) {
		...
		Player player = new Player();
		int countryID = player.getIdOfCountryControllingAllStates(myStateList);
		countryID = player.getCountryControllingAllCapitals(myStateList);
		...
		if (countryID == -1) {
			...
			player = map.getPlayerByID(countryID);
			gameStatus = true;
			Logger.logMessage("\n");
			Logger.logMessage("All states or capitals are controlled by : "
					+ player.getCountryName() + ". Game cannot proceed.");
		}
		return gameStatus;
	}
	...
	public void productionPhase(int countryID, int technologyLevel)
		...
		Resource countryResource = player.getResourceOfCountry(countryStates);
		Logger.logMessage("***************************************************");
		Logger.logMessage("Turn " + getCountryTurn(countryID));
		Logger.logMessage("***************************************************");
		Logger.logMessage("---------------------------------------------------");
		Logger.logMessage(player.getCountryName());
		Logger.logMessage("---------------------------------------------------");
		Logger.logMessage("---------------------------------------------------");
		Logger.logMessage("PRODUCTION PHASE : ");
		Logger.logMessage("---------------------------------------------------");
		Logger.logMessage("Capital : "
				+ player.getCountryCapital(countryID, countryStates)
						.getStateName());

		Technology countryTechnology = player.getTechnology();
		Logger.logMessage(player.getCountryName() + " controls Resources : ["
				+ player.getAllResourcesOfCountryStr(countryStates) + "]");
		Logger.logMessage(player.getCountryName() + " controls Technology : ["
		...
		if (continentOwnedByCountry.size() > 0) {
			...
			Logger.logMessage("Other than these states, "
					+ player.getCountryName() + " controls : " + normalStates
					+ " states = " + normalStates + " reinforcements");
		} else {
			totalStates = countryStates.size();
			Logger.logMessage(player.getCountryName() + " controls : "
		...
		State capital = player.getCountryCapital(countryID, myStateList);

		player.addArmyDetail(army);
		capital.addArmyDetail(army);
		player.setProductionNumber(totalStates);
		player.setProductionType(army.armyName());

		Logger.logMessage("Total production from " + capital.getStateName()
				+ " factory : " + player.getProductionNumber() + " "
				+ player.getProductionType());

		int countryIndex = player.getPlayerIndexByCountryID(countryID,
				myPlayerList);
		...
		myPlayerList.set(countryIndex, player);
		map.setMyPlayerList(myPlayerList);
		updateMap();
	}
	...
	private int getCountryTurn(int countryId) {
		int countryTurn = 0;
		for (int i = 0; i < playerTurnSequence.size(); i++) {
			if (countryId == playerTurnSequence.get(i).getCountryID()) {
				countryTurn = i + 1;
				break;
			}
		}
	...
	private void loggingForInitialSetup() {
		Logger.logMessage("Player selected " + myPlayerList.size()
				+ " countries");
		/*
